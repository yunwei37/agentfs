...
When one path succeeds (e.g., tests pass), its changes must be applied atomically to the shared workspace. All sibling paths must be automatically invalidated, since they now operate on stale state and could silently corrupt the committed result. Failed paths must be discardable without residual side effects.
