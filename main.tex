...
\begin{table}[t]
	...
	\resizebox{\columnwidth}{!}{%
		\begin{tabular}{lccccc}
			\toprule
			Feature              & OverlayFS & Btrfs/ZFS & DM-Snap & DAXFS  & \sys   \\
			\midrule
	...
	\smallskip
	{\scriptsize $^{*}$Stacked overlays supported since Linux~5.0, but without commit semantics.}
\end{table}
...
With file descriptor caching to amortize per-request \texttt{open()}/\texttt{close()} overhead, regular FUSE mode achieves 1,655\,MB/s read throughput (19\% of native). The read gap reflects the FUSE kernel-to-userspace roundtrip on each request. Write throughput under FUSE slightly exceeds native because FUSE's \texttt{writeback\_cache} mode absorbs writes into the page cache before forwarding them to the daemon; for a 50\,MB workload on 8\,GB RAM this measures page-cache speed, not durable I/O. Passthrough mode (\texttt{FOPEN\_PASSTHROUGH}) bypasses the daemon for unmodified files, achieving 7,236\,MB/s read throughput (4.4$\times$ regular FUSE, 82\% of native). These numbers are sufficient for typical AI agent workloads, which are dominated by LLM API latency (100\,ms--10\,s per request) rather than local file I/O.
